...
سیم کشی هر المان نسبت به برد اردوینو:
...
-- Pin1    ->    ArduinoPin21(ArduinoPinA3)
...
#define Motor_PowerOnValue 100

#define MotorDrive_Pin 21
#define MotorDrive_PowerOffValue LOW
#define MotorDrive_PowerOnValue HIGH
...
#define Time_FromHomeSection_ToPlasticSection 250
#define Time_FromPlasticSection_ToHomeSection 250
#define Time_FromHomeSection_ToOtherSection 500
#define Time_FromOtherSection_ToHomeSection 500
...
#define IR_Pin 1
#define IR_DefaultStatus LOW
#define IR_TriggeredStatus HIGH
...
const unsigned long int ProgramTime_Empty = (unsigned long int)18000;
const unsigned long int ProgramTime_Full = (unsigned long int)6000;

const char *PhoneNumber = "+9892029692690";
...
void setup(void) {
  ...
  // Configuring And Turning On GSM Module
  pinMode((uint8_t)GSM_PWR_Pin, (uint8_t)OUTPUT);
  digitalWrite((uint8_t)GSM_PWR_Pin, (uint8_t)HIGH);
  digitalWrite((uint8_t)GSM_PWR_Pin, (uint8_t)LOW);
  delay(2000);
  digitalWrite((uint8_t)GSM_PWR_Pin, (uint8_t)HIGH);
  ...
  pinMode((uint8_t)MotorDrive_Pin, (uint8_t)OUTPUT);
  digitalWrite((uint8_t)Motor_Pin_One, (uint8_t)LOW);
  digitalWrite((uint8_t)Motor_Pin_Two, (uint8_t)LOW);
  digitalWrite((uint8_t)MotorDrive_Pin, (uint8_t)LOW);
  // Configuring Keys
...
void loop(void) {
  ...
  if (digitalRead((uint8_t)PIR_Pin) == (int)PIR_TriggeredStatus) {
    ...
    if (IsAvailable == true) {
      ...
      if ((!IsFull)) {
        // This Code Will Run For 180 Seconds
        TheTime = millis();
      ...
      // This Code Will Run If The Trash Can Is Fulled
        ...
        // This Code Will Run For 60 Seconds
        TheTime = millis();
...
void Opening_HeadOfBox(void) {
  for (unsigned short int Counter = (unsigned short int)ServoMotor_StartPointDegree; Counter <= (unsigned short int)ServoMotor_EndPointDegree; Counter += (unsigned short int)2) {
    ServoMotor.write((int)Counter);
    delay((unsigned long int)10);
  }
}

// Configuring Body Of The Declared Function
void Closing_HeadOfBox(void) {
  for (unsigned short int Counter = (unsigned short int)ServoMotor_EndPointDegree; Counter >= (unsigned short int)ServoMotor_StartPointDegree; Counter -= (unsigned short int)2) {
    ServoMotor.write((int)Counter);
    delay((unsigned long int)10);
  }
}
    ...
    case ((unsigned short int)Glass_SectionOfBox):
      {
        if ((unsigned short int)Glass_SectionOfBox == (unsigned short int)Home_SectionOfBox) {
          ...
          digitalWrite((uint8_t)MotorDrive_Pin, (uint8_t)MotorDrive_PowerOnValue);
          ...
          digitalWrite((uint8_t)MotorDrive_Pin, (uint8_t)MotorDrive_PowerOffValue);
...
void Showing_HelpMessageToUser(void) {
  ...
  Display.setCursor((uint8_t)0, (uint8_t)2);
  Display.print("  Blue : Plastical  ");
  Display.setCursor((uint8_t)0, (uint8_t)3);
  Display.print("    Blue : Other    ");
}
...
void Sending_Message(const char *PhoneNumber, const char *Message) {
  char *ConfigPhoneNumber;
  sprintf(ConfigPhoneNumber, "AT+CMGS=\"%s\"", PhoneNumber);

  GSM.println("AT");
  delay((unsigned long int)1000);
